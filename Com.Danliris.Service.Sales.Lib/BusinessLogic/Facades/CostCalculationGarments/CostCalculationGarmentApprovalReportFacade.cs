...
{
    public class CostCalculationGarmentApprovalReportFacade : ICostCalculationGarmentApprovalReport
    {
        private readonly SalesDbContext DbContext;
        private readonly DbSet<CostCalculationGarment> DbSet;
        private IdentityService IdentityService;
        private CostCalculationGarmentApprovalReportLogic CostCalculationGarmentApprovalReportLogic;

        public CostCalculationGarmentApprovalReportFacade(IServiceProvider serviceProvider, SalesDbContext dbContext)
        {
            this.DbContext = dbContext;
            this.DbSet = this.DbContext.Set<CostCalculationGarment>();
            this.IdentityService = serviceProvider.GetService<IdentityService>();
            this.CostCalculationGarmentApprovalReportLogic = serviceProvider.GetService<CostCalculationGarmentApprovalReportLogic>();
        }
        ...
        public Tuple<MemoryStream, string> GenerateExcel(string filter = "{}")
        {
            ...
            var Query = CostCalculationGarmentApprovalReportLogic.GetQuery(filter);
            ...
            {
                int index = 0;
                foreach (CostCalculationGarmentApprovalReportViewModel item in data)
                {
                            index++;
                    ...
                    result.Rows.Add(index, item.RO_Number, item.StaffName, item.UnitName, CfrmDate, item.Section, item.BuyerCode, item.BuyerName, item.BrandCode, item.BrandName,
                                    item.Comodity, item.Article, ShipDate, QtyOrder, item.UOMUnit, item.ValidatedKadiv, ValidDate);
                }
            }
            ...
            string fileName = string.Concat("Validasi CC Garment", ".xlsx");

            return Tuple.Create(streamExcel, fileName);

        }

        public Tuple<List<CostCalculationGarmentApprovalReportViewModel>, int> Read(int page = 1, int size = 25, string filter = "{}")
        {
            var Query = CostCalculationGarmentApprovalReportLogic.GetQuery(filter);
            var data = Query.ToList();
